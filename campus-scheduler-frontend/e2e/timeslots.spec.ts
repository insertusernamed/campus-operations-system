...
test.describe('Time Slots CRUD', () => {
	...
	test('should create a time slot', async ({ page }) => {
		...
		// Scope to <main> so navbar role select does not conflict.
		await main.locator('select').selectOption('FRIDAY');
		await main.locator('input[type="time"]').first().fill('14:00');
		await main.locator('input[type="time"]').nth(1).fill('15:00');
		await main.locator('input[placeholder="e.g., Period 1"]').fill('Period 5');
