			...
			<template #default>
				<div v-if="selectedRequest" class="space-y-4">
					<div>
						...
						<div class="text-sm text-gray-900">{{ selectedRequest.schedule.course.code }} - {{
							selectedRequest.schedule.course.name }}</div>
					</div>
					<div>
						<div class="text-sm text-gray-500">Reason</div>
						<div class="text-sm text-gray-900">
							{{ reasonLabel(selectedRequest.reasonCategory) }}
							<div v-if="selectedRequest.reasonDetails" class="text-xs text-gray-500">{{
								selectedRequest.reasonDetails }}</div>
						</div>
					</div>
					<div>
					...
					<div v-if="impact" class="bg-gray-50 border border-gray-200 text-gray-700 p-3 rounded">
						...
						<div class="mt-2 text-xs text-gray-600">
							...
							<p>
								...
								/
								{{ selectedRequest?.schedule.timeSlot ?
									timeslotsService.formatTimeSlot(selectedRequest.schedule.timeSlot) : 'Unknown time' }}
